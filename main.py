...
""")
    ...
    async def send_pm(ctx, *, message: str):
        ...
        async for member in guild.fetch_members(limit=None):
            if member != ctx.author and member.id != bot.user.id:
                try:
                    ...
                    print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} {red}Status: {white}{str(e)} {red}User: {white}{member.name}#{member.discriminator} ({member.id}) {red}Message: {white}{message}")
